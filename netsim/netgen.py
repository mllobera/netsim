...
def setup(df):
    ...
    if 'mix' not in colnames:
        ...
        msg.append('\n WARNING: no mix column found, mix column has been created!!')
        ...
        if not valid:
           ...
           # group_mix for the first group always set to 0 
            ...
            if not valid:
                c_df.loc[c_df['group'] == groups[0], 'mix'] = 0
                msg.append('WARNING: mix column - resetting first group mix to 0')
            ...
            for g in groups:
                ...
                if len(unique_mix) > 1:
                    ...
                    msg.append('WARNING: mix column - resetting mix in group '+str(g)+
...
def __paths_table(ids, iteration, df_out, option='close'):
    '''
    ...
    option: string
        defines the type of network that will be generated. Currently the options are as follows:
           - *close* (*default*): this network is generated by defining a path between each location id and the next location
           ...
           - *cummulative*: this network is generated by defining a first path between the first two
    ...
    if option == 'cummulative':
        ...
        for o_indx in range(len(ids)):
            ...
            for d_indx in range(len(d_indxs)):
                df_out.loc[len(df_out.index)] = [ids[o_indx], d_indxs[d_indx], iteration]        
